...
func debugWSPayload(enabled bool, flow, upstream, proto string, payload []byte) {
	_ = enabled
	...
	log.Printf("[ws] payload flow=%s upstream=%q proto=%q len=%d preview_hex=%s", flow, upstream, proto, len(payload), hex.EncodeToString(preview))
}
...
func pumpH3ToBackend(ctx context.Context, s io.ReadWriter, bws *websocket.Conn, lim config.Limits, st *sessionTrafficStats, debug bool, upstream, proto string) error {
	...
	flushMessage := func(op byte, msg []byte) error {
		...
		case ws.OpText:
			...
			if err == nil {
				debugWSPayload(debug, "proxy->backend", upstream, proto, msg)
	...
	for {
		...
		case ws.OpText, ws.OpBinary:
			debugWSPayload(debug, "h3->proxy", upstream, proto, f.Payload)
		...
		case ws.OpClose:
			...
			debugWSPayload(debug, "proxy->backend", upstream, proto, websocket.FormatCloseMessage(code, reason))
...
func pumpBackendToH3(ctx context.Context, bws *websocket.Conn, s io.Writer, lim config.Limits, st *sessionTrafficStats, debug bool, upstream, proto string) error {
	bws.SetPingHandler(func(appData string) error {
		debugWSPayload(debug, "backend->proxy", upstream, proto, []byte(appData))
		metrics.Frames.WithLabelValues("h1_to_h3", "ping").Inc()
		metrics.Ctrl.WithLabelValues("ping").Inc()
		debugWSPayload(debug, "proxy->h3", upstream, proto, []byte(appData))
	...
	bws.SetCloseHandler(func(code int, text string) error {
		...
		debugWSPayload(debug, "backend->proxy", upstream, proto, closePayload)
		metrics.Frames.WithLabelValues("h1_to_h3", "close").Inc()
		metrics.Ctrl.WithLabelValues("close").Inc()
		debugWSPayload(debug, "proxy->h3", upstream, proto, closePayload)
	...
	for {
		...
		if err != nil {
			...
			if ce, ok := err.(*websocket.CloseError); ok {
				...
				case websocket.CloseNormalClosure, websocket.CloseGoingAway, websocket.CloseNoStatusReceived:
					...
					debugWSPayload(debug, "proxy->h3", upstream, proto, websocket.FormatCloseMessage(ce.Code, ce.Text))
			...
			if ce, ok := err.(*websocket.CloseError); ok {
				...
				debugWSPayload(debug, "proxy->h3", upstream, proto, websocket.FormatCloseMessage(1011, "backend read error"))
		...
		case websocket.TextMessage:
			debugWSPayload(debug, "backend->proxy", upstream, proto, data)
			...
			debugWSPayload(debug, "proxy->h3", upstream, proto, data)
